...
function part1Implementation(entries: string[], isTest? : boolean, testNumber?:number) {
    ...
    const snapshot = new Cube<string>(maxX+1,maxY+1,maxZ+1,(c)=>'.');
    ...
    for(let i=0; i<X1.length; i++){
        ...
        const z1 = Z1[i];
        const z2 = Z2[i];
        const brickId = isTest? String.fromCharCode(65+i) : i.toString(32);
        console.log(`marking brick '${brickId}' from (${x1},${y1},${z1}) to (${x2},${y2},${z2})`)
        for(let x=x1; x<=x2; x++){
            for(let y=y1; y<=y2; y++){
                for(let z=z1; z<=z2; z++){
                    snapshot.setCell(x, y, z, brickId);
                }
            }
        }
    ...
    snapshot.logToConsole('Y', 0, (c)=> c!);
    snapshot.logToConsole('Y', 1, (c)=> c!);
    snapshot.logToConsole('Y', 2, (c)=> c!);

    snapshot.logToConsole('X', 0, (c)=> c!);
    snapshot.logToConsole('X', 1, (c)=> c!);
    snapshot.logToConsole('X', 2, (c)=> c!);
