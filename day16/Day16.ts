...
function part2Implementation(entries: string[]) {
    ...
    for(let x=0; x<grid.width; x++){
        console.log(`Down from x = ${x}..`)
        maxEnergy = Math.max(maxEnergy, energize({ x, y: 0, direction: 1 }, grid));
        console.log(`Up from x = ${x}..`)
        maxEnergy = Math.max(maxEnergy, energize({ x, y: grid.maxY, direction: 3 }, grid));
    }

    for(let y=0; y<grid.height; y++){
        console.log(`Right from y = ${y}..`)
        maxEnergy = Math.max(maxEnergy, energize({ x:0, y, direction: 0 }, grid));
        console.log(`Left from y = ${y}..`)
        maxEnergy = Math.max(maxEnergy, energize({ x: grid.maxX, y, direction: 2 }, grid));
    }

    return `${maxEnergy}`;
}

function energize(tail: { x: number; y: number; direction: number; }, grid: Grid<string>) : number{
    const energyLevels = new Grid<number>(grid.width, grid.height);
    for(let x=0; x<energyLevels.width; x++){
        for(let y=0; y<energyLevels.height; y++){
            energyLevels.setCell(x,y,0);
        }
    }
    const tails = [tail];  // 0 is left to right, 1 top to bottom, 2, right to left, 3, bottom to top
    let steps = 0;
    do {
        //if (steps % 1000) console.log(`${steps} : TAILS : ${JSON.stringify(tails)}`);
